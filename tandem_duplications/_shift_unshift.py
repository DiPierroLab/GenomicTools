...
def shift_dots(dot_plot, species_data_A, species_data_B, maps_A, maps_B):
    ...
    for chromA in chromsA:
        ...
        for chromB in chromsB:
            ...
            dataAB = dot_plot[(dot_plot[:,0] == chromA)*(dot_plot[:,2] == chromB)][:,np.array(1,3)].astype(int)
            if dotsAB.shape[0] > 0:
                ...
                shifted_dots.append(np.vstack([np.array(N*[chromA]),shifted_dots_A,np.array(N*[chromB]),shifted_dots_B]).T.astype(int))
    ...
    for block in synteny_blocks:
        ...
        chromA, chromB = block[0,np.array([0,2])]
                ...
                for nb, b in enumerate(block):
                    cc_block_A = inv_cc_maps_A[chromA][unshift_maps_A[chromA][b[1]]]
                    cc_block_B = inv_cc_maps_B[chromB][unshift_maps_B[chromB][b[3]]]
