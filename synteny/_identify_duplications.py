...
def identify_duplications(condensed_blocks, species_data, chrom_info, nanosynteny_minsize, unshift_blocks = False):
    """
    Identify multi-gene duplications from synteny blocks generated by comparing a genome with itself.

    Input:
        - condensed_blocks: list of N X 4 arrays, microsynteny blocks with TDECs, N varies from block to block
        - species_data: N X 12 array, species data
        - chrom_info: dictionary, chromosome information
        - nanosynteny_minsize: integer, minimum size of nanosynteny (usually 3 genes)
        - unshift_blocks: boolean, unshift the blocks from TDECs to genes?

    Output:
        - duplications: list of N X 4 arrays, multi-gene duplications, N varies from block to block
        - palindromes: list of N X 4 arrays, palindromes (see block_palindrome description), N varies from block to block
        - palindromoids: list of N X 4 arrays, palindromoids (see block_palindromoid description), N varies from block to block
    """
...
def get_species_time_to_ancestor(ancestor, tree, time_tree):
    """
    Find the time to an ancestral species in a phylogenetic tree.

    Input:
        - ancestor: string, the name of an internal node of the species tree
        - tree: biopython Tree object, species tree
        - time_tree: biopython Tree object, species tree with actual time as branch lengths (assumed to be in MYA)

    Output:
        - name: string, name of ancester
        - t: float, time to ancestor from present (in MYA)
    """
...
def get_single_node_time_to_mrca(sp, tree, time_tree):
    """
    This seems to be a vestigial copy of get_species_time_to_ancestor, a paralog that has neither gained a new function
    nor undergone mutational decay into a pseudo-function.

    Input:
        - sp: string, the name of an internal node of the species tree
        - tree: biopython Tree object, species tree
        - time_tree: biopython Tree object, species tree with actual time as branch lengths (assumed to be in MYA)
...
def condense_og_sequence(og_sequence):
    """
    Condense a sequence of orthogroups into a condensed sequence.

    Input:
        - og_sequence: list of integers, a list of orthogroups, ordered as the corresponding genes appear in the genome

    Output:
        - og_sequence_condensed: list of integers, each tandem duplication block will only appear as a single orthogroup index
    """
...
def generate_node_to_time_map(tree, time_tree):
    """
    Generate a mapping of the nodes of a phylogenetic tree to its properties in actual time (MYA).
    ...
    Output:
        - node_to_time_map: dictionary, mapping each node to another dictionary with information about its age and distance from the root.
    """
    node_to_time_map = {}
...
def duplication_timing_gene_pair_DLCpar(geneA, geneB, og, gene_tree, map_dups_to_species):
    """
    Determine the most recent common ancestor of a duplication according to the DLCpar results.

    Input:
        - geneA: string, name of gene A in the specified orthogroup
        - geneB: string, name of gene B, a paralog of gene A, in the specified orthogroup
        - og: integer, orthogroup index
        - gene_tree: biopython Tree object, the gene tree for the specified orthogroup
        - map_dups_to_species: dictionary, a mapping from duplicaiton to species

    Output:
        - gene_mrca_sp: string, the most recent common ancester of genes A and B
        - support: float, the support of the duplication (see OrthoFinder documentation)
    """
...
def duplication_timing_DLCpar(condensed_duplication, sp, all_species_data, gene_tree_data, map_dups_to_species, node_to_time_map):
    """
    Get the consensus DLCpar timing of a multi-gene duplication.
    
    Input:
        - condensed_duplication: N X 4 array, duplication synteny block with TDECs
        - sp: string, name of species
        - all_species_data: dictionary, a dictionary containing the species data of all species with binomial species names as keys
        - gene_tree_data: dictionary, container for strings encoding the gene trees of all orthogroups. This needs to hold
          strings (parsed by the function parse_tree) to save on memory.
        - map_dups_to_species: dictionary, mapping duplications to species
        - node_to_time_map: dictionary, mapping each node to time

    Output:
        - DLCpar_node: string, the consensus duplication node from DLCpar
        - DLCpar_sps_from_root: float, the substitutions per site from the root to the DLCpar_node
        - DLCpar_time: float, the time to DLCpar_node in MYA (that is, from present)
        - DLCpar_support: float, consensus support of this duplication
    """
...
def duplication_timing_parsimony(condensed_duplication, sp, all_species_data, species_tree, node_to_time_map):
    """
    Infer the duplication node of multi-gene duplication using parsimony.

    Input:
        - condensed_duplication: N X 4 array, duplication synteny block with TDECs
        - sp: string, name of species
        - all_species_data: dictionary, a dictionary containing the species data of all species with binomial species names as keys
        - species_tree: biopython Tree object, species tree
        - node_to_time_map: dictionary, mapping each node to time
        
    Output:
        - ancestor: string, name of inferred duplication node of species tree
    """
...
def nanosynteny_duplication_timing(condensed_duplications, sp, all_species_data, gene_tree_data, species_tree, map_dups_to_species, node_to_time_map):
    """
    Get nanosynteny-based parsimony timings for all multi-gene duplications.

    Input:
        - condensed_duplications: list of N X 4 array, duplication nanosynteny blocks with TDECs, N varies from block to block
        - sp: string, name of species
        - all_species_data: dictionary, a dictionary containing the species data of all species with binomial species names as keys
        - gene_tree_data: dictionary, container for strings encoding the gene trees of all orthogroups. This needs to hold
          strings (parsed by the function parse_tree) to save on memory.
    ...
    Output:
        - nanosynteny_duplication_timing: dictionary, with all estimated timing data for multi-gene duplications from nanosynteny-based parsimony
    """
...
def microsynteny_duplication_timing(nanosynteny_duplication_timing, condensed_self_microsynteny):
    """
    Get microsynteny-based parsimony timings for all multi-gene duplications.

    Input:
        - nanosynteny_duplication_timing: dictionary, the output of nanosynteny_duplication_timing
        - condensed_self_microsynteny: list of N X 4 array, duplication microsynteny blocks with TDECs, N varies from block to block
    
    Output:
        - microsynteny_duplication_timing: dictionary, with all estimated timing data for multi-gene duplications from microsynteny-based parsimony
    """
