...
def run_synteny_identification(dot_plot, species_data_A, species_data_B, chrom_info_A, chrom_info_B, params):
    ...
    for chromA in chromsA:
        for chromB in chromsB:
            ...
            convolved_dots_AB = identify_blocks_chrom_pair(shifted_dots_AB, minsize = params['condensed_block_minsize'], filter_maxdist=True,maxdist=1)
            if len(convolved_dots_AB) == 0:
                continue
            else:
                extended_dots_AB = extend_dots(shifted_dots_AB, convolved_dots_AB, params['dot_maxdist'])
                blocks_AB = identify_blocks_chrom_pair(extended_dots_AB, minsize = params['condensed_block_minsize'],filter_maxdist=True,maxdist=params['dot_maxdist'])
                i = 0
                while True:
    ...
    unshifted_blocks = unshift_synteny_blocks(synteny_blocks, maps_A, maps_B)
